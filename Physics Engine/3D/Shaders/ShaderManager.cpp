...
std::string ShaderManager::readShader(const std::string& fileName) {
	...
	try {
		...
		printf("Could not read shader at %s\n%s %i", fileName.c_str(), __FILE__, __LINE__);
		return "";
	}
	return shaderCode;
}
...
void ShaderManager::createProgram(const std::string& shaderName, const std::string& vertexShaderFileName, const std::string& fragmentShaderFileName) {
	...
	if(!linkResult) {
		...
		printf("Shader %s  failed to link.\n%s\n%s %i", shaderName.c_str(), logString.c_str(), __FILE__, __LINE__);
		return;
	}
	...
	for(int i = 0; i < count; i++) {
		...
		std::cout << "\t" << toString(type) << " " << name << " loc: " << loc << std::endl;
	}
